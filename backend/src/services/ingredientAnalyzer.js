...
class IngredientAnalyzer {
  ...
  /** Vision paragraph looks like the real INGREDIENTS panel header (not marketing copy). */
  _visionLineIsIngredientHeader(lineText) {
    const t = String(lineText || '').trim();
    if (!/\bingredients?\b/i.test(t)) return false;

    const afterHeader = t
      .replace(/^\s*(?:our\s+|the\s+)?ingredients?\s*[:\-]?\s*/i, '')
      .trim();

    if (
      afterHeader &&
      /^(?:are|is|was|were|from|that|which|we|thoughtfully|crafted)\b/i.test(afterHeader)
    ) {
      return false;
    }
    if (
      afterHeader &&
      !/,/.test(afterHeader) &&
      /\b(?:quality|animal sources?|commitment|recipes|sourced|crafted|wellbeing|protein-?rich)\b/i.test(
        afterHeader
      )
    ) {
      return false;
  ...
  _visionLineIsIngredientTail(lineText) {
    return /(?:^|\b)(?:calorie\s+content|made\s+in|manufactured\s+in|produced\s+in|packaged\s+in|guaranteed\s+analysis|AAFCO|feeding\s+(?:guide|instructions?)|distributed\s+by|www\.)/i.test(
      String(lineText || '')
    );
  }

  /**
   * Rebuild ingredient narrative from Vision paragraph boxes (top-to-bottom, left-to-right).
   * @param {Array<{ text: string, ymin: number, xmin: number }>} visionLines
   * @returns {string}
   */
  buildIngredientNarrativeFromVisionLines(visionLines) {
    if (!Array.isArray(visionLines) || visionLines.length < 2) return '';

    const sorted = [...visionLines]
    ...
    for (let i = 0; i < sorted.length; i++) {
      if (this._visionLineIsIngredientHeader(sorted[i].text)) {
    ...
    for (const startIdx of headerIndices) {
      let endIdx = sorted.length;
      for (let i = startIdx + 1; i < sorted.length; i++) {
        if (this._visionLineIsIngredientTail(sorted[i].text)) {
          endIdx = i;
          break;
        }
      }

      const section = sorted.slice(startIdx, endIdx);
      let joined = section.map(l => String(l.text).trim()).join(' ');
      ...
      if (score > bestScore) {
        bestScore = score;
        bestJoined = joined;
        bestMeta = { lineCount: section.length };
      }
    }

    if (bestJoined && bestMeta) {
      console.log(
        `📐 [Ingredients] Y-sorted: ${sorted.length} Vision lines → ${bestMeta.lineCount} rows, commas=${this._countTopLevelCommas(bestJoined)}, len=${bestJoined.length}, score=${bestScore}`
      );
    }
