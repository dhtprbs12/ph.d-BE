-- Split puppy_kitten → puppy | kitten (run after 05 if products table exists)
-- Requires: 01_helpers.sql (optional — this file uses plain ALTER/UPDATE)
...
-- ── product_reviews.pet_age_group (skip if table missing) ──
...
SELECT 'puppy_kitten split complete' AS status;

SELECT target_life_stage, target_pet_type, COUNT(*) AS cnt
FROM products
GROUP BY target_life_stage, target_pet_type
ORDER BY target_life_stage, target_pet_type;
